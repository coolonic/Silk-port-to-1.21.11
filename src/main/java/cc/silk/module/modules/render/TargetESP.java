...
public final class TargetESP extends Module {
    ...
    private void onRender3D(Render3DEvent e) {
        ...
        // NOTE (Batch 2): RenderSystem.enableBlend/blendFunc/setShaderTexture/setShader are removed
        // in 1.21.11 — the entire tessellator/BufferRenderer draw pipeline must be rewritten using
        // the new RenderPipeline/RenderPass API.  These GlStateManager calls are a temporary shim.
        GlStateManager._enableBlend();
        // GL_SRC_ALPHA=770, GL_ONE=1, GL_ZERO=0
        GlStateManager._blendFuncSeparate(0x0302, 1, 1, 0);
        ...
        for (var entity : mc.world.getEntities()) {
            ...
            double tPosX = entity.lastX + (entity.getX() - entity.lastX) * tickDelta - cam.x;
            double tPosY = entity.lastY + (entity.getY() - entity.lastY) * tickDelta - cam.y + yOff;
            double tPosZ = entity.lastZ + (entity.getZ() - entity.lastZ) * tickDelta - cam.z;
