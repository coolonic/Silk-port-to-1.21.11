...
public final class Trajectories extends Module {
    ...
    private List<Vec3d> calculateTrajectory(ItemStack stack) {
        ...
        double playerX = mc.player.lastX + (mc.player.getX() - mc.player.lastX) * tickDelta;
        double playerY = mc.player.lastY + (mc.player.getY() - mc.player.lastY) * tickDelta;
        double playerZ = mc.player.lastZ + (mc.player.getZ() - mc.player.lastZ) * tickDelta;

        float yaw = mc.player.lastYaw + (mc.player.getYaw() - mc.player.lastYaw) * tickDelta;
        float pitch = mc.player.lastPitch + (mc.player.getPitch() - mc.player.lastPitch) * tickDelta;
    ...
    private HitResult raycast(Vec3d start, Vec3d end) {
        ...
        var entityHit = ProjectileUtil.getEntityCollision(
                ...
                entity -> !entity.isSpectator() && entity != mc.player, 1.0f);
