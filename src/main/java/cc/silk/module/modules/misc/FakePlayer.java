...
public class FakePlayer extends Module {
    ...
    private void popTotem() {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            mc.world.addParticleClient(ParticleTypes.TOTEM_OF_UNDYING,
                    fakePlayer.getX() + offsetX,
                    fakePlayer.getY() + offsetY,
                    fakePlayer.getZ() + offsetZ,
                    0, 0.1, 0);
    ...
    private void spawnFakePlayer() {
        ...
        try {
            // Copy skin properties so the fake player looks like the real one.
            // In newer authlib versions properties() may be immutable – skip gracefully.
            profile.properties().putAll(original.properties());
        } catch (UnsupportedOperationException ignored) {
            // Immutable PropertyMap in this authlib version; fake player will use default skin
        }
