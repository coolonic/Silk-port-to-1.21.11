...
public final class TriggerBot extends Module {
    ...
    private void tick(TickEvent event) {
        ...
        if (AutoMace.wantsToAttack) return;
        ...
        if (waitingForReaction && timerReactionTime.hasElapsedTime(currentReactionDelay, true)) {
            if (critMode.getMode().equals("Strict")) {
                if (!mc.player.isOnGround() && !mc.player.isClimbing()) {
                    ...
                    boolean movingForward = KeyUtils.isKeyPressed(GLFW.GLFW_KEY_W);

                    if (sprintCrit.getValue() && isSprinting && movingForward && cooldownReady) {
                        // Drop sprint proactively while airborne so the crit window can be hit
                        mc.options.forwardKey.setPressed(false);
                        mc.player.setSprinting(false);
                        waitingForSprintDrop = true;
                        sprintDropTick = 0;
                    } else if (canCrit() && cooldownReady) {
    ...
    private void handleSprintDrop() {
        ...
        if (!mc.player.isSprinting()) {
            // Sprint is dropped; wait for the crit window before attacking
            if (canCrit() || sprintDropTick >= 10) {
                if (hasTarget(target) && samePlayerCheck(target)) {
                    attack();
                }
                waitingForReaction = false;
        ...
        } else if (sprintDropTick >= 3) {
            // Sprint didn't drop in time; attack anyway
            if (hasTarget(target) && samePlayerCheck(target)) {
                attack();
            }
            waitingForReaction = false;
