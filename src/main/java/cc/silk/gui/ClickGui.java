...
public final class ClickGui extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Matrix3x2fStack mat2d = context.getMatrices();
        mat2d.pushMatrix();
        mat2d.translate(centerX, centerY);
        mat2d.scale(animationManager.getScaleAnimation(), animationManager.getScaleAnimation());
        mat2d.translate(-centerX, -centerY);
    ...
    private void renderModuleContent(DrawContext context, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        { int bc = applyAlpha(theme.muted(), 120).getRGB(); int bx=x+PADDING,by=moduleY,bw=width-PADDING*2,bh=MODULE_HEIGHT; context.fill(bx,by,bx+bw,by+1,bc); context.fill(bx,by+bh-1,bx+bw,by+bh,bc); context.fill(bx,by,bx+1,by+bh,bc); context.fill(bx+bw-1,by,bx+bw,by+bh,bc); }
    ...
    private void renderConfigButton(DrawContext context, int x, int y, int width, int height, String text, Color baseColor, int mouseX, int mouseY) {
        ...
        Theme theme = ThemeManager.getTheme(ClickGUIModule.theme.getMode());
        smallFont.drawString(new MatrixStack(), text, textX, textY, theme.text());
    }
    ...
    public boolean charTyped(CharInput input) {
        ...
        return eventHandler.handleCharTyped(chr, input.modifiers()) || super.charTyped(input);
    }
