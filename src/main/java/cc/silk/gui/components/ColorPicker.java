...
public class ColorPicker {
    ...
    private void renderColorSquare(DrawContext context, int x, int y) {
        ...
        // 1.21.11: DrawContext.getMatrices() returns Matrix3x2fStack with no peek().getPositionMatrix() — use identity
    ...
    private void renderColorPreview(DrawContext context, int x, int y) {
        ...
        int bc = applyAlpha(theme.muted(), 200).getRGB();
        context.fill(x, y, x + PREVIEW_SIZE, y + 1, bc);
        context.fill(x, y + PREVIEW_SIZE - 1, x + PREVIEW_SIZE, y + PREVIEW_SIZE, bc);
        context.fill(x, y, x + 1, y + PREVIEW_SIZE, bc);
        context.fill(x + PREVIEW_SIZE - 1, y, x + PREVIEW_SIZE, y + PREVIEW_SIZE, bc);
    }
    ...
    private void finishRendering(BufferBuilder buffer, DrawContext context, int x, int y, int width, int height) {
        // 1.21.11: BufferRenderer.drawWithGlobalProgram() removed — discard built buffer (TODO: use RenderPipeline)
        try (var built = buffer.end()) { /* no-op stub */ } catch (Exception ignored) {}
        GlStateManager._disableBlend();
        context.getMatrices().popMatrix();
        Theme theme = ThemeManager.getTheme(ClickGUIModule.theme.getMode());
        int bc = applyAlpha(theme.muted(), 200).getRGB();
        context.fill(x, y, x + width, y + 1, bc);
        context.fill(x, y + height - 1, x + width, y + height, bc);
        context.fill(x, y, x + 1, y + height, bc);
        context.fill(x + width - 1, y, x + width, y + height, bc);
    }
